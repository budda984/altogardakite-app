...
interface DateInputProps {
  ...
  hint?: string;
  required?: boolean;
  value: string; // ISO yyyy-mm-dd o vuoto - controlled
...
/**
 * Auto-formattazione progressiva: trasforma "01052026" in "01/05/2026"
 * inserendo le slash mentre l'utente digita.
 */
function autoFormat(raw: string): string {
  // Estrai solo le cifre, scarta tutto il resto
  const digits = raw.replace(/\D/g, '');
  if (digits.length === 0) return '';
  if (digits.length <= 2) return digits;
  if (digits.length <= 4) return `${digits.slice(0, 2)}/${digits.slice(2)}`;
  // Massimo 8 digit
  const t = digits.slice(0, 8);
  return `${t.slice(0, 2)}/${t.slice(2, 4)}/${t.slice(4)}`;
}

/**
 * Parsa "gg/mm/aaaa" in ISO "yyyy-mm-dd". Richiede ANNO A 4 CIFRE.
 * Niente piu' conversione 2-cifre -> 4-cifre.
 */
...
export function DateInput({
  ...
  // Sincronizza testo col valore controllato esterno
  useEffect(() => {
  ...
  const handleTextChange = (newText: string) => {
    ...
    setText(formatted);
    // Aggiorna il valore esterno SOLO se la stringa e' completa (10 caratteri)
  ...
  const handleTextBlur = () => {
    // Al blur prova a parsare. Se l'input e' incompleto (es. "5/3"), niente.
    if (text.length === 10) {
